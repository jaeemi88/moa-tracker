// 진로모아관리(기관·프로그램별) 레코드를 저장/조회/삭제하는 함수입니다.
...
export default async function handler(req, res) {
  ...
  const itemKey = (id) => `tracker_program:${t}:${id}`;
  ...
  if (req.method === 'GET') {
    ...
    try {
      ...
      if (!value) return res.status(404).json({ error: '프로그램을 찾을 수 없습니다.' });
  ...
  if (req.method === 'POST') {
    ...
    if (!record || !record.id || !record.orgName || !record.programName) {
    ...
    try {
      ...
      const summary = JSON.stringify({
        ...
        date: record.date, participantCount: record.participantCount,
        avgSatisfaction: record.avgSatisfaction
      });
  ...
  if (req.method === 'DELETE') {
    ...
    try {
      ...
      // 해당 프로그램의 설문 응답 인덱스도 함께 정리
      await client.del(`tracker_survey_index:${t}:${id}`);
